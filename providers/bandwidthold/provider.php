...
class providers_bandwidthold_provider extends providers_aprovider {
    ...
    private function _number_process($rate_center, &$zerocheck, &$number_obj, $update) {
        $numbers = array();
        $arr_numbers = array();
        $quantity = 0;
    ...
    private function _dowload_numbers($state, $update = false) {
        ...
        foreach ($rate_centers as $rate_center) {
            ...
            if ($zerocheck->$rate_center <= $this->_settings->max_zerocheck) {
                echo "First attempt... \n";
                if (!$this->_number_process($rate_center, $zerocheck, $number_obj, $update)) {
                    echo "First attempt failed... Let's try again\n";
                    // Yeah, it seems that on the Bandwidth, 1/2 request fails.
                    // So just retrying... You never know
                    $this->_number_process($rate_center, $zerocheck, $number_obj, $update);
                }
